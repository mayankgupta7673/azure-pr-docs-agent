...
async function run() {
  try {
    ...
    const config = {
      ...
      updatePrDescription: core.getInput('update-pr-description') === 'true'
    };
...
async function handlePullRequest(context, octokit, config) {
  ...
  if (config.updatePrDescription) {
    await updatePrDescription(octokit, context, pullRequest, {
      filesProcessed: fileDiffs.length,
      docPaths: docPaths,
...
/**
 * Update PR description with documentation summary
 */
async function updatePrDescription(octokit, context, pullRequest, summary) {
  try {
    const sectionIdentifier = '<!-- azure-integration-doc-summary -->';
    const timestamp = new Date().toISOString();
    
    // Build the documentation summary section
    const docSummary = `
${sectionIdentifier}
...
✅ **Files Processed:** ${summary.filesProcessed}  
📄 **Documentation Generated:** ${summary.docPaths.map(p => `[\`${p}\`](${p})`).join(', ')}  
⏰ **Last Updated:** ${timestamp}

### Changed Azure Files:
${summary.azureFiles.map(f => `- \`${f.filename}\` (${f.status}) - +${f.additions}/-${f.deletions} lines`).join('\n')}

---

*Auto-generated by [Azure Integration Doc Agent](https://github.com/mayankgupta7673/azure-integration-doc-agent) 🤖*
`;

    // Get current PR body
    let currentBody = pullRequest.body || '';
    
    // Remove existing documentation summary if present
    if (currentBody.includes(sectionIdentifier)) {
      const parts = currentBody.split(sectionIdentifier);
      currentBody = parts[0].trim();
      
      // Find the end marker (next --- or end of string)
      if (parts.length > 1) {
        const afterMarker = parts[1];
        const endMatch = afterMarker.match(/\n---\n\*Auto-generated by.*?\*\n/s);
        if (endMatch) {
          currentBody += afterMarker.substring(endMatch.index + endMatch[0].length);
        }
      }
    }
    
    // Append new documentation summary
    ...
    core.info('📝 PR description updated with documentation summary');
  } catch (error) {
    core.warning(`Failed to update PR description: ${error.message}`);
  }
}
